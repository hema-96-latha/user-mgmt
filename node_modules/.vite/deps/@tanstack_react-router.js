...
// node_modules/@tanstack/router-core/node_modules/@tanstack/store/dist/esm/alien.js
var ReactiveFlags = ((ReactiveFlags22) => {
  ReactiveFlags22[ReactiveFlags22["None"] = 0] = "None";
  ReactiveFlags22[ReactiveFlags22["Mutable"] = 1] = "Mutable";
  ReactiveFlags22[ReactiveFlags22["Watching"] = 2] = "Watching";
  ReactiveFlags22[ReactiveFlags22["RecursedCheck"] = 4] = "RecursedCheck";
  ReactiveFlags22[ReactiveFlags22["Recursed"] = 8] = "Recursed";
  ReactiveFlags22[ReactiveFlags22["Dirty"] = 16] = "Dirty";
  ReactiveFlags22[ReactiveFlags22["Pending"] = 32] = "Pending";
  return ReactiveFlags22;
})(ReactiveFlags || {});
function createReactiveSystem({
  ...
  function unlink22(link32, sub = link32.sub) {
    const dep = link32.dep;
    const prevDep = link32.prevDep;
    const nextDep = link32.nextDep;
    const nextSub = link32.nextSub;
    const prevSub = link32.prevSub;
    if (nextDep !== void 0) {
  ...
  function propagate22(link32) {
    let next = link32.nextSub;
    let stack;
    top: do {
      const sub = link32.sub;
      let flags = sub.flags;
      if (!(flags & (4 | 8 | 16 | 32))) {
        sub.flags = flags | 32;
      } else if (!(flags & (4 | 8))) {
      ...
      } else if (!(flags & (16 | 32)) && isValidLink(link32, sub)) {
      ...
      if (flags & 1) {
        ...
        if (subSubs !== void 0) {
          const nextSub = (link32 = subSubs).nextSub;
          if (nextSub !== void 0) {
  ...
  function checkDirty22(link32, sub) {
    let stack;
    let checkDepth = 0;
    let dirty = false;
    top: do {
      ...
      if (sub.flags & 16) {
        ...
        if (link32.nextSub !== void 0 || link32.prevSub !== void 0) {
          stack = { value: link32, prev: stack };
        }
        link32 = dep.deps;
        sub = dep;
        ++checkDepth;
  ...
  function shallowPropagate22(link32) {
    do {
      const sub = link32.sub;
    ...
    } while ((link32 = link32.nextSub) !== void 0);
  }
  function isValidLink(checkLink, sub) {
    let link32 = sub.depsTail;
    while (link32 !== void 0) {
      if (link32 === checkLink) {
        return true;
      }
      link32 = link32.prevDep;
    }
    return false;
  }
}
...
// node_modules/@tanstack/router-core/node_modules/@tanstack/store/dist/esm/atom.js
...
// node_modules/@tanstack/router-core/node_modules/@tanstack/store/dist/esm/batch.js
function batch(fn) {
  try {
    startBatch();
    fn();
...
function createBrowserHistory(opts) {
  ...
  const queueHistoryAction = (type, destHref, state) => {
    ...
    if (!scheduled) {
      scheduled = Promise.resolve().then(() => flush4());
    }
  };
...
var ReactiveFlags2 = ((ReactiveFlags22) => {
  ReactiveFlags22[ReactiveFlags22["None"] = 0] = "None";
  ReactiveFlags22[ReactiveFlags22["Mutable"] = 1] = "Mutable";
  ReactiveFlags22[ReactiveFlags22["Watching"] = 2] = "Watching";
  ReactiveFlags22[ReactiveFlags22["RecursedCheck"] = 4] = "RecursedCheck";
...
function createReactiveSystem2({
  update,
  notify,
  unwatched
}) {
...
var queuedLength2 = 0;
var queued2 = [];
var { link: link3, unlink: unlink3, propagate: propagate3, checkDirty: checkDirty3, shallowPropagate: shallowPropagate3 } = createReactiveSystem2({
  update(node) {
    if (node.depsTail !== void 0) {
      return updateComputed2(node);
    } else {
      return updateSignal2(node);
    }
  },
  notify(effect2) {
    var _a;
    let insertIndex = queuedLength2;
    let firstInsertedIndex = insertIndex;
    do {
      queued2[insertIndex++] = effect2;
      effect2.flags &= -3;
      effect2 = (_a = effect2.subs) == null ? void 0 : _a.sub;
      if (effect2 === void 0 || !(effect2.flags & 2)) {
        break;
    ...
    queuedLength2 = insertIndex;
    while (firstInsertedIndex < --insertIndex) {
      const left = queued2[firstInsertedIndex];
      queued2[firstInsertedIndex++] = queued2[insertIndex];
      queued2[insertIndex] = left;
    }
  },
  unwatched(node) {
    if (!(node.flags & 1)) {
      effectScopeOper2.call(node);
    } else if (node.depsTail !== void 0) {
      node.depsTail = void 0;
      node.flags = 1 | 16;
      purgeDeps3(node);
...
function updateSignal2(s) {
  s.flags = 1;
  return s.currentValue !== (s.currentValue = s.pendingValue);
}
function effectScopeOper2() {
  this.depsTail = void 0;
  this.flags = 0;
  purgeDeps3(this);
  const sub = this.subs;
...
var queuedEffects2 = [];
var { link: link4, unlink: unlink4, propagate: propagate4, checkDirty: checkDirty4, shallowPropagate: shallowPropagate4 } = createReactiveSystem2({
  update(atom) {
    return atom._update();
  },
  // eslint-disable-next-line no-shadow
  notify(effect2) {
    queuedEffects2[queuedEffectsLength2++] = effect2;
    effect2.flags &= ~ReactiveFlags2.Watching;
  },
  unwatched(atom) {
    if (atom.depsTail !== void 0) {
      atom.depsTail = void 0;
      atom.flags = ReactiveFlags2.Mutable | ReactiveFlags2.Dirty;
      purgeDeps4(atom);
    }
  }
});
var queuedEffectsLength2 = 0;
function purgeDeps4(sub) {
  const depsTail = sub.depsTail;
  let dep = depsTail !== void 0 ? depsTail.nextDep : sub.deps;
  while (dep !== void 0) {
    dep = unlink4(dep, sub);
...
var useTags = () => {
  ...
  const links = useRouterState({
    select: (state) => {
      var _a2;
      const constructed = state.matches.map((match) => match.links).filter(Boolean).flat(1).map((link5) => ({
        tag: "link",
        attrs: {
          ...link5,
          nonce
